...
// #############################################################################
	...
	// Number of Jets for ttDKF Permutations
  	ttDKF_njet_UserValue = 4 ;   	     // for ttbar+H : >= 4 jets and <= Maximum Number of Jets in Event
        ...
        // luminosity
        Luminosity = 4700./1000.;       // evaluated for Top_GRL_K.xml for 7TeV	
        //Luminosity = 13873.6/1000.;   // evaluated for Top_GRL_K.xml for 8TeV
        //Luminosity = 20339.8/1000.;   // evaluated for Top_GRL_K.xml FDS (Full Data Set) for 8TeV

  	// GRL to use
  	GRLXMLFile="TopGRL.xml";  

	// b-tag weight cut
        BTagCut = 0.601713; // 7TeV
        // BTagCut = 0.772; // 8TeV
...
// #############################################################################
	...
	for(Int_t i = 0; i <= LastCut; ++i){
		...
		if( LeptonVec.size()>1 && MyGoodJetVec.size()>1 && myNBtag <= 2 ){
                   ...
                   // define histogram id for all ZDD combinations
                   int nH_id ;
                   int nHpre ;
                   int nH_idGe2 ;
                   int nHpreGe2 ;
		   ...
		   // ==============================================================================
                   ...
                   // Mll counter
                   if( fabs( xVar - 91.) < 10. ){  // inside the Z window
...
//        cout << "C9 a) runNumber=" << RunNumber << " eventNumber=" << EveNumber << endl;
        ...
        //=============================================
        //=============================================
        // C9) b)  Z Mass cut: |M(ll)-91|>=12 GeV  ====
        //=============================================
        //=============================================
        if( lepSample==21 ){ // ee sample
          if( fabs(ll.M()/GeV-91.) < 12. ) return;
        }
        if( lepSample==22 ){ // mumu sample
          if( fabs(ll.M()/GeV-91.) < 12. ) return;
        }

...
// #############################################################################
	...
	// Number of Jets for ttDKF Permutations
  	ttDKF_njet_UserValue = 4 ;   	     // for ttbar+H : >= 4 jets and <= Maximum Number of Jets in Event
        ...
        // luminosity
        // Luminosity = 13900./1000.; // evaluated for Top_GRL_K.xml
...
// #############################################################################
  ...
  // print info to the log
    ...
    // =====New Mini Ntuples @ 8 TeV TRCR12_01_07 (ttbar and ttH with top quark status code 155)
	...
	for(Int_t i = 0; i <= LastCut; ++i){
		...
		if ( myNjet >= 4 && myNBtag >= 4 ) 	histo[i][328]->Fill(   12., Weight);
                ...
                if(LeptonVec.size()>1 && MyGoodJetVec.size()>1 ){
                   ...
                   if ( (LeptonVec[0].isb)*(LeptonVec[1].isb) > 0 ) nH = 800+400;  // check if it is SS
                   ...
                   // Mll counter
                   if( fabs( xVar - 91.) < 8. ){  // inside the Z window
...
// #############################################################################
        ...
        //   ***Read Jared Total Data Minintuple***
        //   ***1) for Egamma Stream require: ElectronTrigger==1 and alldata_ele==1
        //   ***2) for Muon   Stream require:     MuonTrigger==1 and alldata_muo==1
        //=============================================
        if(Weight==0) return;
...
//        cout << "C8 b) runNumber=" << RunNumber << " eventNumber=" << EveNumber << endl;
        ...
        // C9)  Z Mass cuts                    ========
	//     Selection is sub-divided in two steps
	//     a) Z Mass cut 1:    M(ll) > 60 GeV (ee,mm)
        //     b) Z Mass cut 2:|M(ll)-91|> 10 GeV (ee,mm)
        //=============================================
        // C9) a) Mass(l+,l-) cut: Mll > 60 GeV
        //=============================================
        if( lepSample==21 ){ // ee sample
          if( ll.M()/GeV <= 60. ) return;
        }
        if( lepSample==22 ){ // mumu sample
          if( ll.M()/GeV <= 60. ) return;
        }
...
//        cout << "C9 a) runNumber=" << RunNumber << " eventNumber=" << EveNumber << endl;
        ...
        //=============================================
        //=============================================
        // C9) b)  Z Mass cut: |M(ll)-91|>=10 GeV  ====
        //=============================================
        //=============================================
        if( lepSample==21 ){ // ee sample
          if( fabs(ll.M()/GeV-91.) < 10. ) return;
        }
        if( lepSample==22 ){ // mumu sample
          if( fabs(ll.M()/GeV-91.) < 10. ) return;
        }

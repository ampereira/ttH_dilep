...
void ttH_dilep::second_DoCuts() {
	...
	for(Event::event_counter = 0; Event::event_counter < events.size(); Event::event_counter++) {
        cout << "Event: " << Event::event_counter << " - " LastCut << endl; 
		LastCut++; // LastCut=22
		//        cout << "C12 runNumber=" << RunNumber << " eventNumber=" << EveNumber << endl;
...
void ttH_dilep::ttDilepKinFitDICE(){
    // #############################################################################
    //
...
void ttH_dilep::ttDilepKinFit(){
    ...
    int _HasSolution = 0;
    ttDKF_Best_Sol best;
    bool last_input = false;    // the last input of an event
    ...
    for (unsigned counter = 0; counter < inputs.size() * dilep_iterations; ++counter) {
        ...
        if (task_id == (int) task_id || first) {
            ...
            if (first) {
                first = false;
                Event::event_counter = di.getEventID();
            }
        }
        ...
        // Checks if this is the last input from that event
        if (di.getEventID() != inputs[task_id + 1].getEventID())
            last_input = true;
